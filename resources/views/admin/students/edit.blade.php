...
<div class="max-w-2xl mx-auto">
    ...
    <x-card>
        <form action="{{ route('admin.students.update', $student) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-4" x-data="{ isPkl: {{ old('is_pkl', $student->is_pkl) ? 'true' : 'false' }} }">
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_pkl" value="1" x-model="isPkl" {{ old('is_pkl', $student->is_pkl) ? 'checked' : '' }} class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
                ...
                <div x-show="isPkl" x-cloak class="rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-4">
                    ...
                    <x-input label="Tempat PKL" name="tempat_pkl" :error="$errors->first('tempat_pkl')" value="{{ old('tempat_pkl', $student->pkl->tempat_pkl ?? null) }}" />
                    <div>
                        ...
                        <select id="pembimbing_id" name="pembimbing_id" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500">
                            ...
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher->id }}" @selected(old('pembimbing_id', $student->pkl->pembimbing_id ?? null) == $teacher->id)>{{ $teacher->user->name ?? '-' }} ({{ $teacher->nip }})</option>
                            @endforeach
                        </select>
                        @error('pembimbing_id')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-input label="Tanggal Mulai" name="pkl_start_date" type="date" :error="$errors->first('pkl_start_date')" value="{{ old('pkl_start_date', isset($student->pkl->start_date) ? $student->pkl->start_date->toDateString() : null) }}" />
                        <x-input label="Tanggal Selesai" name="pkl_end_date" type="date" :error="$errors->first('pkl_end_date')" value="{{ old('pkl_end_date', isset($student->pkl->end_date) ? $student->pkl->end_date->toDateString() : null) }}" />
                    </div>
                    <x-select label="Status PKL" name="pkl_status" :options="['PLANNED' => 'PLANNED', 'ACTIVE' => 'ACTIVE', 'COMPLETED' => 'COMPLETED', 'CANCELLED' => 'CANCELLED']" placeholder="Pilih status" :error="$errors->first('pkl_status')" value="{{ old('pkl_status', $student->pkl->status ?? 'ACTIVE') }}" />
                </div>
            </div>
